...
static inline SimpAssembler* init_simpasm(uint32_t kmer_size, uint32_t rd_len, uint32_t strand, uint32_t min_ol, float min_sm, uint32_t max_mm, int allow_gap){
	SimpAssembler *sa;
	sa = malloc(sizeof(SimpAssembler));
	sa->sra  = sr_init_sdb(kmer_size, 4, rd_len);
	sr_set_align_parameters(sa->sra, strand, min_ol, min_sm, max_mm, allow_gap);
	sr_set_filter_parameters(sa->sra, 3, 1024, 1);
	sa->rds  = init_seqv(16);
...
static inline void print_ctgs(SimpAssembler *sa){
	SimpContigInfo *ctg;
	uint32_t i, j;
	for(i=0;i<count_ctgv(sa->ctgs);i++){
		ctg = ref_ctgv(sa->ctgs, i);
		if(ctg->closed) continue;
		printf("%u\t%s\n", ctg->len, ctg->seq->string);
		for(j=0;j<count_u32list(ctg->sids);j++) printf("%u\n", get_u32list(ctg->sids,  j));
	}
	printf("----------------\n");
	fflush(stdout);
}
...
static inline void simple_assemble(SimpAssembler *sa){
	...
	int exists;
	sr_ready_sdb(sa->sra);
	for(i=0;i<sa->sra->n_idx;i++) sr_index_sdb(sa->sra, i);
	clear_sr_hitv(sa->aux->hits);
	for(i=0;i<sa->sra->n_rd;i++) sr_align_sdb(sa->sra, i, sa->aux);
	//for(i=0;i<count_sr_hitv(sa->sra->hits);i++){
		//hit = ref_sr_hitv(sa->sra->hits, i);
		//sr_output_hit(sa->sra, hit, stdout, 1);
	//}
	qsort(as_array_sr_hitv(sa->aux->hits), count_sr_hitv(sa->aux->hits), sizeof(SR_AlnHit), cmp_sr_alnhit);
	clear_u64hash(sa->r2r);
	clear_sr_hitv(sa->ols);
	for(i=0;i<count_sr_hitv(sa->aux->hits);i++){
		hit = ref_sr_hitv(sa->aux->hits, i);
