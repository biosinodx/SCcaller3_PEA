...
uint32_t asm_ef(FileReader *in, FILE *out, uint32_t min_ol, float min_sm, uint32_t min_read, uint32_t max_read){
	...
	while((n_col = fread_table(in)) != -1){
		if(n_col == 0) continue;
		eid = atoi(get_col_str(in, 0));
		if(eid != ef_id){
			ef_id = eid;
			ret ++;
			reverse_dna(get_col_str(in, 1), get_col_len(in, 1));
			if(ef){
				...
				reset_ef(ef, ef_id, get_col_str(in, 1), get_col_len(in, 1), min_ol, min_sm);
				add_read2ef(ef, get_col_str(in, 2), seqid, get_col_len(in, 2), rank);
			} else {
				ef = init_ef(ef_id, get_col_str(in, 1), get_col_len(in, 1), min_ol, min_sm);
				add_read2ef(ef, get_col_str(in, 2), seqid, get_col_len(in, 2), rank);
			}
		}
		//rank  = atoi(get_col_str(in, 1));
		rank  = 1;
		if (vec_size(ef->rds) <= max_read) {  //magic number 400
			add_read2ef(ef, get_col_str(in, 1), seqid, get_col_len(in, 1), rank);
			add_read2ef(ef, get_col_str(in, 2), seqid, get_col_len(in, 2), rank);
			seqid ++;
		}
	}
...
int ef_usage(){
	printf(
"Local assemble fragments around break point sites\n"
"Usage: ef [options]\n"
...
" -r <int>    Minium reads to execute assembly [4]\n"
" -R <int>    Maxium reads to execute assembly [400]\n"
"\n"
	);
	return 1;
}
