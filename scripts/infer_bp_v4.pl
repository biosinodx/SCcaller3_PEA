...
while (<IN>) {
	...
	system("${main_path}/bin/samtools", "view", $tbam, $chr1.":".$beg."-".$fin, "-o", "$PID.regionsam2.tmp");
#	`samtools view $tbam $e[0]:$e[1]-$e[1] > regionsam2.tmp`;
	...
	close $fh;
	system("${main_path}/bin/samtools", "view", $nbam, $chr1.":".$beg."-".$fin, "-o", "$PID.regionsam2.tmp");
#	`samtools view $nbam $e[0]:$e[1]-$e[1] > regionsam2.tmp`;
	...
	close $fh;
	system("${main_path}/bin/samtools", "view", $tbam, $chr2.":".$beg2."-".$fin2, "-o", "$PID.regionsam2.tmp");
#	`samtools view $tbam $e[0]:$e[1]-$e[1] > regionsam2.tmp`;
	open $fh , "$PID.regionsam2.tmp" or die $!;
	...
	close $fh;
	system("${main_path}/bin/samtools", "view", $nbam, $chr2.":".$beg2."-".$fin2, "-o", "$PID.regionsam2.tmp");
#	`samtools view $nbam $e[0]:$e[1]-$e[1] > regionsam2.tmp`;
	open $fh , "$PID.regionsam2.tmp" or die $!;
