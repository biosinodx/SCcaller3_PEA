...
my @ids = (); # ids of kmers and ids of pairs
my @sz = ();
for (my $i = 0; $i < $id; $i++) {
	push @ids, $i;
	push @sz, 1;
...
while (<IN1>) {
	...
	if ($. % 4 == 2) {
		...
		$seq1 = $_;
		{
			...
			chomp;
			$seq2 = $_;
			$id2pair{$id} = $seq1."_".$seq2."_".$pair_name;
			if (($id - $kmer_num) % 10000 == 0) {
				printf STDERR "handled [%d] pairs\n", $id - $kmer_num;
			}
			push @ids, $id;
			push @sz, 1;
...
sub print_pair {
	...
	foreach my $pair (@$group) {
		my ($seq1, $seq2, $p_name) = split /_/, $pair;
		print join("\t", ($gid, $seq1, $seq2, $p_name)), "\n";
	}
}
