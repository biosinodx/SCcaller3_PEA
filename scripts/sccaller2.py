...
def main():
    ...
    def write_vcf_head(output_file, my_format, fasta_file, bulk, minvar):
        ...
        head_str = "##fileformat=VCFv4.3\n" \
                   ...
                   "homozygous SNV respectively\">\n".format(time.strftime("##fileDate=%Y%m%d", time.localtime()),
                                                             ...
                                                             minvar)
        if bulk != "":
            head_str = head_str + "##FORMAT=<ID=SO,Number=1,Type=String,Description=\"Whether there is any read in bulk supporting the alt genotype.\">\n"
    ...
    def check_env():
        ...
        if float(sys.version[:3]) != 2.7:
            print "\x1b[1;4;31mCRITICAL: Python version must be 2.7!\n\x1b[0m"
            sys.exit(1)
        # check numpy version >=1.10.2
        numpy_version = np.version.version
        numpy_version_list = [int(i) for i in numpy_version.split(".")]
        if len(numpy_version_list) < 3:
            warning_msg = "\x1b[1;4;31mNumpy version error. {}\x1b[0m".format(numpy_version)
            logging.warning(warning_msg)
            print warning_msg
        else:
            if numpy_version_list[0] >= 1 and numpy_version_list[1] >= 10 and numpy_version_list[2] >= 2:
                pass
            else:
                warning_msg = "\x1b[1;4;31mYour numpy version is {}, you better consider to change to " \
                              "numpy 1.10.2 or later version.\x1b[0m".format(numpy_version)
                logging.warning(warning_msg)
                print warning_msg
        # check pysam version >= 0.15.0
        pysam_version = pysam.__version__  # type:str
        pysam_version_list = [int(i) for i in pysam_version.split(".")]
        if len(pysam_version_list) < 2:
            warning_msg = "\x1b[1;4;31mPysam version error. {}\x1b[0m".format(pysam_version)
            logging.warning(warning_msg)
            print warning_msg
        else:
            if pysam_version_list[1] < 15:
                warning_msg = "\x1b[1;4;31mYour pysam version is {}, you better consider to change to " \
                              "pysam 0.15.0 or later version.\x1b[0m".format(pysam_version)
                logging.warning(warning_msg)
                print warning_msg

        # check samtools version >= 1.3
        process_samtools = Popen(["samtools --version"], shell=True, stdout=PIPE)
        samtools_version = process_samtools.stdout.readline()
        if len(samtools_version) == 0:
            warning_msg = "\x1b[1;4;31mSomething is wrong with samtools. You need to fix it or reinstall samtools.\x1b[0m"
            logging.warning(warning_msg)
            print warning_msg
            exit(1)
        samtools_version = samtools_version.split(" ")
        if len(samtools_version) < 2:
            warning_msg = "\x1b[1;4;31mSamtools version error. {}\x1b[0m".format(samtools_version)
            logging.warning(warning_msg)
            print warning_msg
        else:
            samtools_version = samtools_version[1]
            samtools_version_list = [int(i) for i in samtools_version.split(".")]
            if len(samtools_version_list) < 2:
                warning_msg = "\x1b[1;4;31mSamtools version error.\x1b[0m"
                logging.warning(warning_msg)
                print warning_msg
            else:
                if samtools_version_list[0] == 1 and samtools_version_list[1] >= 3 or samtools_version_list[0] > 1:
                    pass
                else:
                    warning_msg = "\x1b[1;4;31mYour samtools version is {}, you better consider to change to" \
                                  " version later than 1.3. Samtools 1.9 is recommended.\x1b[0m".format(samtools_version)
                    logging.warning(warning_msg)
                    print warning_msg
    ...
    logging.basicConfig(
        ...
        filemode="w")
    check_env()
    logging.warning("\x1b[30m >>>>>>>>If you can see this, please use less -r log_file_name. <<<<<<<<<\x1b[0m")
